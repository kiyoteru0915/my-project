...
import { Task, Project, SubFolder, ViewType } from '../types'
...
const deadline1 = new Date(todayDate)
deadline1.setDate(deadline1.getDate() + 30)

const deadline2 = new Date(todayDate)
deadline2.setDate(deadline2.getDate() + 14)

const deadline3 = new Date(todayDate)
deadline3.setDate(deadline3.getDate() + 7)

const deadline4 = new Date(todayDate)
deadline4.setDate(deadline4.getDate() + 45)
...
const sampleTasks: Task[] = [
  {
    ...
    subFolderId: 'sf-pr',
    title: 'プレスリリース作成',
    description: '新製品ローンチに向けたプレスリリースの作成と配信準備',
  ...
  },
  {
    ...
    subFolderId: 'sf-sns',
    title: 'SNSキャンペーン企画',
    description: '夏季キャンペーンのSNS戦略立案とコンテンツ計画',
  ...
  },
  {
    ...
    projectId: 'proj-marketing',
    title: '市場調査レポート',
    description: '競合他社分析と市場トレンドのレポートまとめ',
    deadline: today,
  ...
  },
  {
    ...
    title: 'UI/UXデザインレビュー',
    description: '新機能のUI/UXデザインのレビューとフィードバック',
    deadline: format(deadline3, 'yyyy-MM-dd'),
    estimatedMinutes: 60,
    subTaskSegments: generateSubTaskSegments(60),
  ...
  },
  {
    ...
    projectId: 'proj-product',
    title: 'APIドキュメント整備',
    description: '新しいAPIエンドポイントのドキュメント作成と整備',
    deadline: format(deadline4, 'yyyy-MM-dd'),
    estimatedMinutes: 120,
    subTaskSegments: generateSubTaskSegments(120),
...
interface StoreState {
  ...
  addTask: (task: Omit<Task, 'id' | 'createdAt' | 'timerElapsedSeconds' | 'milestones' | 'subTaskSegments'> & { generateBreakdown?: boolean }) => void
    ...
    (set, get) => ({
      ...
      deleteSubFolder: (id) => {
        set((state) => ({
          ...
          tasks: state.tasks.map((t) =>
            t.subFolderId === id ? { ...t, subFolderId: undefined } : t
          ),
      ...
      addTask: (taskData) => {
        ...
        if (generateBreakdown && rest.deadline) {
          ...
          const deadline = new Date(rest.deadline)
          milestones = generateMilestones(rest.title, startDate, deadline)
        }
              ...
              milestones: t.milestones.map((m) =>
                m.id === milestoneId
                  ? {
                      ...
                      completedDate: !m.completed ? format(new Date(), 'yyyy-MM-dd') : undefined,
                    }
                  : m
              ),
            }
      ...
      completeSegment: (taskId, segmentId) => {
        set((state) => ({
          tasks: state.tasks.map((t) => {
            ...
            return {
              ...t,
              subTaskSegments: t.subTaskSegments.map((s) =>
                s.id === segmentId ? { ...s, completed: true } : s
              ),
            }
          }),
        }))
      ...
      stopTimer: () => {
        ...
        if (timerState.taskId) {
          get().updateTask(timerState.taskId, {
            timerElapsedSeconds: timerState.totalElapsedSeconds,
          })
        }
        set({
          timerState: {
        ...
        if (newSegmentElapsed >= segmentDurationSeconds) {
          ...
          if (allDone) {
            get().updateTask(timerState.taskId, {
              ...
              timerElapsedSeconds: newTotalElapsed,
            })
            set({
              timerState: {
                ...timerState,
          ...
          set({
            timerState: {
              ...timerState,
              segmentElapsedSeconds: newSegmentElapsed,
              totalElapsedSeconds: newTotalElapsed,
            },
          })
        }
