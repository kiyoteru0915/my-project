...
- 各部署・クライアントの詳細指針はそれぞれの `CLAUDE.md` を参照する
...
- 新規クライアント追加時は `Business/` 直下にフォルダを作成し、CLAUDE.md を整備する
